...
namespace components {
    extern uint32_t ScalerGetBufferStrideInBytes(const Scaler::Buffer* buffer);
    extern uint32_t ScalerGetBufferStrideInPixels(const Scaler::Buffer* buffer);
    extern void ScalerFillBufferStride(Scaler::Buffer* buffer);

    static bool IsBufferContinus(const Scaler::Buffer* buffer) {
    ...
    static void OpenCVMatToBuffer(const cv::Mat& mat, Scaler::Buffer* buffer, bool copy = true) {
        if (!buffer) return;
    ...
    static void OpenCVBufferToMat(const Scaler::Buffer* buffer, cv::Mat* mat, bool copy = false) {
        if (!buffer || !mat) return;

        uint32_t stride = ScalerGetBufferStrideInPixels(buffer);
        if (buffer->color <= Scaler::ColorFormat::YUV_NV21) {
            if (IsBufferContinus(buffer) && !copy) {
                *mat = cv::Mat(buffer->height * 3 / 2, stride, CV_8UC1, buffer->data[0]);
                mat->cols = buffer->width;
                mat->step = stride;
            }
            else {
                // LOG(INFO) << "OpenCVBufferToMat() copy";
    ...
    static bool OpenCVConvertColor(const cv::Mat& src, Scaler::ColorFormat src_color,
        cv::Mat* dst, Scaler::ColorFormat dst_color) {
        ...
        if (code == -1) {
            LOG(ERROR) << "OpenCVConvertColor() unsupport color convert type";
            return false;
        }
        if (src_color <= Scaler::ColorFormat::YUV_NV21 && dst_color <= Scaler::ColorFormat::YUV_NV21) {
            if (src_color == Scaler::ColorFormat::YUV_I420) {
                OpenCVI420ToYUVsp(&src_mat, dst_color == Scaler::ColorFormat::YUV_NV12);
            }
            else if (dst_color == Scaler::ColorFormat::YUV_I420) {
                OpenCVYUVspToI420(&src_mat, src_color == Scaler::ColorFormat::YUV_NV12);
            }
            else {
        ...
        if (dst_color == Scaler::ColorFormat::YUV_NV12 || dst_color == Scaler::ColorFormat::YUV_NV21) {
            OpenCVI420ToYUVsp(dst, dst_color == Scaler::ColorFormat::YUV_NV12);
        }
        return true;
    }

    static void OpenCVCopy(const Scaler::Buffer* src, Scaler::Buffer* dst) {
    ...
    /*
    static void OpenCVDumpMat(const std::string &file, const cv::Mat &mat, Scaler::ColorFormat color) {
      cv::Mat dump_mat = mat.clone();
      OpenCVConvertColor(dump_mat, color, &dump_mat, Scaler::ColorFormat::BGR);
      cv::imwrite(file, dump_mat);
    }

    static void OpenCVDumpBuffer(const std::string &file, const Scaler::Buffer *buffer) {
      cv::Mat mat;
      OpenCVBufferToMat(buffer, &mat, true);
      OpenCVConvertColor(mat, buffer->color, &mat, Scaler::ColorFormat::BGR);
      cv::imwrite(file, mat);
    }
    */
    static void OpenCVResize(const cv::Mat& src, cv::Mat* dst, uint32_t dst_width, uint32_t dst_height,
        Scaler::ColorFormat color) {
        if (!dst) return;
        if (color >= Scaler::ColorFormat::BGR) {
            cv::resize(src, *dst, cv::Size(dst_width, dst_height));
        }
        else {
            if (color != Scaler::ColorFormat::YUV_I420) {
                cv::Mat* mat = const_cast<cv::Mat*>(&src);
                OpenCVYUVspToI420(mat, color == Scaler::ColorFormat::YUV_NV12);
            }
            dst_height = dst_height * 3 / 2;
            cv::resize(src, *dst, cv::Size(dst_width, dst_height));
            if (color != Scaler::ColorFormat::YUV_I420) {
                OpenCVI420ToYUVsp(dst, color == Scaler::ColorFormat::YUV_NV12);
            }
        }
    }

    bool OpenCVProcess(const Scaler::Buffer* src, Scaler::Buffer* dst) {
        if (!src || !dst) return false;

        if (src->color > Scaler::ColorFormat::RGB || dst->color > Scaler::ColorFormat::RGB) {
            LOG(ERROR) << "OpenCVProcess() unsupport color";
            return false;
        }
        ...
        if (src->color <= Scaler::ColorFormat::YUV_NV21 && dst->color >= Scaler::ColorFormat::BGR &&
            (dst->width % 2 == 1 || dst->height % 2 == 1 || dst->width > src->width || dst->height > src->height)) {
            // do color convert first and then resize
            if (!OpenCVConvertColor(src_mat, src->color, &dst_mat, dst->color)) {
                LOG(ERROR) << "OpenCVProcess() convert color 1 failed";
                return false;
            }
            ...
            src_mat = dst_mat;
            if (!OpenCVConvertColor(src_mat, src->color, &dst_mat, dst->color)) {
                LOG(ERROR) << "OpenCVProcess() convert color 2 failed";
                return false;
            }
            OpenCVMatToBuffer(dst_mat, dst);
