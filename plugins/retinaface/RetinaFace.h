...
public:
    ...
    vector<vector<FaceDetectInfo>> detectBatchImages(vector<cv::Mat> imgs, float threshold=0.5);
    std::vector<FaceDetectInfo> detect(const Mat &img, float threshold=0.5, float scales=1.0);
private:
