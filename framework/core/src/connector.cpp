...
namespace easysa {
    ...
    Connector::Connector(const size_t conveyor_count, size_t conveyor_capacity) {
        ...
        for (size_t i = 0; i < conveyor_count; ++i) {
            ...
            LOG_IF(FATAL, nullptr == conveyor) << "[core]:" << "Connector::Connector()  new Conveyor failed.";
    ...
    Conveyor* Connector::GetConveyorByIdx(int idx) const {
        LOG_IF(FATAL, idx < 0) << "[core]:" << "Connector::GetConveyorByIdx() idx < 0.";
        LOG_IF(FATAL, idx >= static_cast<int>(conveyors_.size())) << "[core]:" << "Connector::GetConveyorByIdx() idx outpace conveyors size";
    ...
    void Connector::EmptyDataQueue() {
        ...
        for (size_t conveyor_idx = 0; conveyor_idx < conveyor_cnt; ++conveyor_idx) {
            GetConveyorByIdx(conveyor_idx)->PopAllDataBuffer(); // disable warning
        }
    }
