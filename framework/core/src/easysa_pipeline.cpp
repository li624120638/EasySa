...
#include "util/easysa_time_utility.hpp"
...
namespace easysa {
    ...
    void Pipeline::StreamMsgHandleFunc() {
        while (!exit_msg_loop_) {
            ...
            if (exit_msg_loop_) {
                LOG(INFO) << "[core]:" << "[" << GetName() << "] stop updating stream message";
                return;
            }
            switch (msg.type) {
    ...
    Pipeline::Pipeline(const std::string& name) : name_(name) {
        ...
        LOG_IF(FATAL, nullptr == event_bus_) << "[core]:" << "Pipeline::Pipeline() failed to alloc EventBus";
        GetEventBus()->AddBusWatch(std::bind(&Pipeline::DefaultBusWatch, this, std::placeholders::_1));

        idxManager_ = new (std::nothrow) IdxManager();
        LOG_IF(FATAL, nullptr == event_bus_) << "[core]:" << "Pipeline::Pipeline() failed to alloc IdxManager";
    }
    ...
    EventHandleFlag Pipeline::DefaultBusWatch(const Event& event) {
        ...
        case EventType::EVENT_ERROR:
            ...
            LOG(ERROR) << "[core]:" << "[" << event.module_name << "]: "
                << "Error: " << event.message;
            ret = EVENT_HANDLE_STOP;
            break;
        case EventType::EVENT_WARNING:
            LOG(WARNING) << "[core]:" << "[" << event.module_name << "]: "
                << "Warning: " + event.message;
            ret = EVENT_HANDLE_SYNCED;
            break;
        case EventType::EVENT_STOP:
            LOG(INFO) << "[core]:" << "[" << event.module_name << "]: "
                << "Info: " << event.message;
            ret = EVENT_HANDLE_STOP;
            break;
        case EventType::EVENT_EOS: {
            LOG(INFO) << "[core]:" << "Pipeline received eos from module " + event.module_name << " of stream " << event.stream_id;
            ret = EVENT_HANDLE_SYNCED;
            break;
        }
        case EventType::EVENT_STREAM_ERROR: {
            ...
            UpdateByStreamMsg(smsg);
            LOG(INFO) << "[core]:" << "Pipeline received stream error from module " + event.module_name
    ...
    bool Pipeline::AddModule(std::shared_ptr<Module> module) {
        ...
        if (modules_map_.find(moduleName) != modules_map_.end()) {
            LOG(WARNING) << "[core]:" << "Module [" << moduleName << "] has already been added to this pipeline";
            return false;
        }

        LOG(INFO) << "[core]:" << "Add Module " << moduleName << " to pipeline";
        module->SetContainer(this);
        if (module->GetId() == INVALID_MODULE_ID) {
            LOG(ERROR) << "[core]:" << "Failed to get module Id";
            return false;
        }
    ...
    std::string Pipeline::LinkModules(std::shared_ptr<Module> up_node, std::shared_ptr<Module> down_node) {
        ...
        if (modules_.find(up_node_name) == modules_.end() ||
            ...
            LOG(ERROR) << "[core]:" << "module has not been added to this pipeline";
            return "";
        }
        ...
        if (!down_node_info.connector) {
            LOG(ERROR) << "[core]:" << "connector is invalid when linking " << link_id;
            return "";
        }
        auto ret = up_node_info.down_nodes.insert(down_node_name);
        if (!ret.second) {
            LOG(ERROR) << "[core]:" << "modules have been linked already";
            return link_id;
        }

        LOG(ERROR) << "[core]:" << "Link Module " << link_id;
    ...
    bool Pipeline::QueryLinkStatus(LinkStatus* status, const std::string& link_id) {
        ...
        if (!con) {
            LOG(ERROR) << "[core]:" << "can not find link according to link id";
            return false;
        }
        if (!status) {
            LOG(ERROR) << "[core]:" << "status cannot be nullptr";
            return false;
        }
    ...
    bool Pipeline::Start() {
        ...
        for (auto& it : modules_map_) {
            if (!it.second->Open(GetModuleParamSet(it.second->GetName()))) {
                ...
                LOG(ERROR) << "[core]:" << it.second->GetName() << " start failed!";
                break;
            }
            else {
        ...
        for (auto& it : modules_) {
            ...
            if ((!parallelism && module_info.input_connectors.size()) ||
                ...
                LOG(ERROR) << "[core]:" << "The parallelism of the first module should be 0, and the parallelism of other modules should be "
                    "larger than 0. "
            ...
            if ((!parallelism && module_info.connector) || (parallelism && !module_info.connector) ||
                ...
                LOG(ERROR) << "[core]:" << "Module parallelism do not equal input Connector's Conveyor number, in module " << node_name;
                Stop();
                return false;
            }
        ...
        LOG(INFO) << "[core]:" << "Pipeline Start";
        LOG(INFO) << "[core]:" << "All modules, except the first module, total  threads  is: " << threads_.size();
        return true;
    }
    ...
    bool Pipeline::Stop() {
        ...
        LOG(INFO) << "[core]:" << "[" << GetName() << "] " << "Stop";
        return true;
    }

    void Pipeline::TransmitData(std::string moduleName, std::shared_ptr<FrameInfo> data) {
        ...
        if (data->IsInvalid()) {
            ...
            UpdateByStreamMsg(msg);
            LOG(WARNING) << "[core]:" << "[" << GetName() << "]" << " got frame error from " << module->name_ <<
            ...
            if (processed_by_all_modules) {
                ...
                while (!connector->IsStopped() && connector->PushDataBufferToConveyor(conveyor_idx, data) == false) {
                    if (connector->GetFailTime(conveyor_idx) % 50 == 0) {
                        ...
                        LOG(INFO) << "[core]:" << "[" << down_node->name_  << " " << conveyor_idx << "] " << "Input buffer is full";
                    }
    ...
    void Pipeline::TaskLoop(std::string node_name, uint32_t conveyor_idx) {
        ...
        //SetThreadName(thread_name, std::this_thread::get_id());
        ...
        while (1) {
            ...
            while (!connector->IsStopped() && data == nullptr) {
                LOG(INFO) << "[core]:" << "[" << instance->name_ << "]" << " There is no avaiable input data"; 
    ...
    int Pipeline::AddModuleConfig(const ModuleConfig& config) {
    ...
    ModuleConfig Pipeline::GetModuleConfig(const std::string& module_name) {
        ModuleConfig config = {};
    ...
    int Pipeline::BuildPipeline(const std::vector<ModuleConfig>& module_configs, const ProfilerConfig& profiler_config) {
        /*TODO,check configs*/
        ...
        for (auto& v : module_configs) {
            ...
            if (!instance) {
                LOG(ERROR) << "[core]:" << "Failed to create module by className(" << v.className << ") and name(" << v.name << ")";
                return -1;
            }
        ...
        for (auto& v : connections_config_) {
            for (auto& name : v.second) {
                if (modules_map_.find(v.first) == modules_map_.end() ||
                    ...
                    LOG(ERROR) << "[core]:" << "Link [" << v.first << "] with [" << name << "] failed.";
                    return -1;
                }
        ...
        for (auto& v : module_configs) {
            if (v.className != "easysa::DataSource" && v.className != "easysa::TestDataSource" &&
                v.className != "easysa::ModuleIPC" &&
                !(((uint64_t)1 << modules_map_[v.name]->GetId()) & linked_id_mask)) {
                LOG(ERROR) << "[core]:" << v.name << " not linked to any module.";
                return -1;
            }
        }
    ...
    int Pipeline::BuildPipelineByJSONFile(const std::string& config_file) {
        std::vector<ModuleConfig> mconfs;
