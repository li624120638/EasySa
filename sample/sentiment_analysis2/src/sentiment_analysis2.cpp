...
bool SentimentAnalysis2::LinkTwoModule(std::string pre_module, std::string post_module,
	const size_t conveyor_count, size_t conveyor_capacity) {
	std::string connector_name = pre_module + "_" + post_module;
	std::shared_ptr<Connector_cfg2> pre_post = std::make_shared<Connector_cfg2>(pre_module, post_module);
...
bool SentimentAnalysis2::InitModuleConnector() {
	// init Videocapture and detector connector
	CALL_LNINK_MODULE(LinkTwoModule, videocapture_, detector_, 1, 25);
	CALL_LNINK_MODULE(LinkTwoModule, detector_, fer_etvh_, 1, 25);
	CALL_LNINK_MODULE(LinkTwoModule, fer_etvh_, osd_, 1, 20);
	return true;
}

void SentimentAnalysis2::ProvideSource() {
...
void SentimentAnalysis2::DetectModule() {
	std::string connector_name = videocapture_ + "_" + detector_;
	auto it = connector_map_.find(connector_name);
	if (it == connector_map_.end()) {
		LOG(ERROR) << "can`t find" << connector_name << "connector";
	...
	// next connector
	std::string next_connector_name = detector_ + "_" + fer_etvh_;
...
void SentimentAnalysis2::FerEtvhModule() {
	std::string connector_name = detector_ + "_" + fer_etvh_;
	auto it = connector_map_.find(connector_name);
	if (it == connector_map_.end()) {
		LOG(ERROR) << "can`t find" << connector_name << "connector";
		return;
	...
	size_t conveyor_idx = std::stoi(stream_id_) % connector->connector_->GetConveyorCount();
	// next connector
	std::string next_connector_name = fer_etvh_ + "_" + osd_;
	auto nit = connector_map_.find(next_connector_name);
	if (nit == connector_map_.end()) {
		LOG(ERROR) << "can`t find" << connector_name << "connector";
		return;
	...
	while (running_) {
		...
		for (auto& it : objs_ptr->objs_) {
			...
			*/
			...
			cv::Mat* roi_etvh = new Mat(src, cv::Rect(left_up_core, right_bottom_core));
			etvh_model_->PushMatToPool(roi_etvh);
		...
		for (auto& it : objs_ptr->objs_) {
			NetOutputs result1;
			etvh_model_->GetResultAsynchr(result1);
			it->AddFeature("etv", result1[0][0]);
			it->AddFeature("eth", result1[1][0]);
...
void SentimentAnalysis2::DrawByObj(cv::Mat& src, std::shared_ptr<easysa::InferObject> obj) {
	...
	std::string v_direction = "V:" + GetLabel(etv, LABEL_TYPE::V_TYPE);
	std::string h_direction = "H:" + GetLabel(eth, LABEL_TYPE::H_TYPE);
	std::string fer_s = "fer:" + GetLabel(fer_result, LABEL_TYPE::FER);
...
std::string SentimentAnalysis2::GetLabel(const std::vector<float>& result, LABEL_TYPE type) {
	/*
	...
	*/
	...
	switch (type)
	{
	case SentimentAnalysis2::FER:
		return fer_status_[index];
		break;
	case SentimentAnalysis2::V_TYPE:
		return v_status_[index];
		break;
	case SentimentAnalysis2::H_TYPE:
...
void SentimentAnalysis2::OsdModule() {
	std::string connector_name = fer_etvh_ + "_" + osd_;
	auto it = connector_map_.find(connector_name);
	if (it == connector_map_.end()) {
		LOG(ERROR) << "can`t find" << connector_name << "connector";
		return;
...
int SentimentAnalysis2::PredictFacePose(const FaceDetectInfo& info) {
...
bool SentimentAnalysis2::InitModule() {
	do {
		// init retinaface
...
void SentimentAnalysis2::Run() {
	do {
		frame_controller_ = new FrameController(frame_rate_);
		if (frame_controller_ == nullptr) break;
		fer_etvh_thread_ = new thread(&SentimentAnalysis2::FerEtvhModule, this);
		if (fer_etvh_thread_ == nullptr) break;
		detect_thread_ = new thread(&SentimentAnalysis2::DetectModule, this);
		if (detect_thread_ == nullptr) break;
		source_thread_ = new thread(&SentimentAnalysis2::ProvideSource, this);
	...
	if (fer_etvh_thread_) {
		delete fer_etvh_thread_;
		fer_etvh_thread_ = nullptr;
	}
	if (detect_thread_) {
...
SentimentAnalysis2::~SentimentAnalysis2() {
	running_ = false;
	if (rf_ != nullptr) {
		delete rf_;
		rf_ = nullptr;
